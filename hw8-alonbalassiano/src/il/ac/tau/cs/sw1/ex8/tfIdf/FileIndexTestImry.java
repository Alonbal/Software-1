...
class FileIndexTestImry {
	...
	void checkGetCosineSimilarity() throws FileIndexException{
		assertEquals(kenDex.getCosineSimilarity("father_time.txt", "crown.txt"),0.02813121395755953, 0.00001);
		assertEquals(kenDex.getCosineSimilarity("money_trees.txt", "these_walls.txt"),0.036476855408337186, 0.00001);
		assertEquals(kenDex.getCosineSimilarity("humble.txt", "money_trees.txt"),0.0073478682703409974, 0.00001);
	}
	@Test
	void checkSimilarFatherTime() throws FileIndexException{
		...
		assertEquals(TFIDFS.get(0), 0.053257961222427146, 0.00001);
		Assertions.assertEquals(TFIDFS.get(1), 0.03245096795720893, 0.00001);
	...
	void checkSimilarCrown() throws FileIndexException {
		...
		assertEquals(TFIDFS.get(0), 0.02813121395755953, 0.000001);
		assertEquals(TFIDFS.get(1), 0.015574629197492888, 0.000001);
	}
